...
module.exports = function (_, opts) {
  ...
  function receiveServer(err, receivedServer) {
    if (err) {
      console.error('Error: Failed to launch server\n' + err.stack + '\n');
      process.exit(1);
    }

    receivedServer.on('error', function (err) {
      b.emit('error', err);
    });

    if (!opts.watch) {
      b.pipeline.get('wrap').on('end', function () {
        receivedServer.close();
      });
    }

    var usedPort = receivedServer.address().port;
    if (!opts.url) {
      opts.url = url.parse('https://localhost:' + usedPort);
    } else {
      opts.url.port = usedPort;
    }
